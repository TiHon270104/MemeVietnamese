...
class Uni_Model(nn.Module):
    ...
    def forward(self, x_text, x_vid):
        return self.network(torch.cat((x_text, x_vid), dim = 1))
...
outputFp = open("MMFoldWiseConcatLessParam.txt", 'a')
...
for text_mod in text_models:
    for image_mod in image_models:
        ...
        for fold in allF:
            ...
            comb = Uni_Model(input_size_text+input_size_image, fc1_hidden_audio, fc2_hidden_audio, 2).to(device)
        ...
        with open("ResultFolder/"+modelName+"_foldWise_res_concat_LessParam.p.p", 'wb') as fp:
